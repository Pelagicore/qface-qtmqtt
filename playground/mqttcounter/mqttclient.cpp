...
MqttClient::MqttClient(QObject *parent)
    ...
    , m_packageId(0)
{
    qCDebug(mqttConnection) << __func__;
...
{
    qCDebug(mqttConnection) << __func__ << "topic:" << topic << " data:" << data;
...
{
    qCDebug(mqttConnection) << __func__ << connected;
...
{
    qCDebug(mqttConnection) << __func__ <<  topic;
}
...
{
    qCDebug(mqttConnection) << __func__ << error;
}
